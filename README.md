...
网站首页包含两个入口：根路径 `/` 提供中文页面，`/en/` 提供英文页面。AetherCloud 开发者预览账户页位于 `/cloud/` 和 `/en/cloud/`，提供 Supabase Auth 登录、密码恢复与修改，以及受 CORS 和 JWT 保护的 AetherCloud API 连通性验证。两种语言必须保持相同的产品边界、链接和页面结构。浏览器只使用公开的 Supabase publishable key，不直接写入 AetherCloud 业务表。
